...
class SlamMode(SerializableEnum):
    ...
    MAP_BROWSER = (
        1  # Just want to reload a map and view it. Don't need loop closing, volume integrator
    )
...
class Slam(object):
    ...
    def set_config_params(self, config: "Config"):
        ...
        if config is not None:
            ...
            far_points_threshold = (
                config.far_points_threshold
            )  # if self.sensor_type != SensorType.MONOCULAR else None
            if far_points_threshold is not None:
                Printer.green(
                    f"Slam: Using far points threshold from config: {far_points_threshold}"
                )
            ...
            if use_fov_centers_based_kf_generation:
                Printer.green(
                    f"Slam: Using fov centers based kf generation from config: {use_fov_centers_based_kf_generation}, max fov centers distance: {max_fov_centers_distance}"
                )
            if self.tracking is not None:
                self.tracking.far_points_threshold = config.far_points_threshold
                self.tracking.use_fov_centers_based_kf_generation = (
                    config.use_fov_centers_based_kf_generation
                )
            ...
            if self.local_mapping is not None:
                ...
                self.local_mapping.use_fov_centers_based_kf_generation = (
                    config.use_fov_centers_based_kf_generation
                )
    ...
    def init_feature_tracker(self, feature_tracker_config):
        ...
        if feature_tracker.tracker_type == FeatureTrackerTypes.LK:
            raise ValueError(
                "SLAM: At present time, you cannot use Lukas-Kanade feature_tracker in this SLAM framework!"
            )
    ...
    def init_volumetric_integrator(self):
        if Parameters.kDoVolumetricIntegration:
            ...
            self.volumetric_integrator = volumetric_integrator_factory(
                self.volumetric_integrator_type,
                self.camera,
    ...
    def track(self, img, img_right, depth, img_id, timestamp=None, mask=None, mask_right=None, prior_pose=None):
        ...
        Returns:
            None
        """
        return self.tracking.track(img, img_right, depth, img_id, timestamp, mask, mask_right, prior_pose)
    ...
    def save_system_state(self, path):
        ...
        config_file_path = (
            path + "/config_info.json"
        )  # redundant, not used on reloading, just for human readability
    ...
    def load_system_state(self, path):
        ...
        with open(map_file_path, "rb") as f:
            ...
            if loaded_sensor_type != self.sensor_type and self.slam_mode == SlamMode.SLAM:
                Printer.yellow(
                    f"SLAM: sensor type mismatch on load_system_state(): {loaded_sensor_type} != {self.sensor_type}"
                )
                # sys.exit(0)
            ...
            if loaded_environment_type != self.environment_type:
                Printer.yellow(
                    f"SLAM: environment type mismatch on load_system_state(): {loaded_environment_type} != {self.environment_type}"
                )
                # sys.exit(0)
            ...
            feature_tracker_config = SerializationJSON.deserialize(
                loaded_json["feature_tracker_config"]
            )
            ...
            loop_detector_config = SerializationJSON.deserialize(
                loaded_json["loop_detector_config"]
            )
        ...
        if self.map is not None and num_keyframes > 0:
            ...
            if camera0.width != self.camera.width or camera0.height != self.camera.height:
                if self.camera.width is not None and self.camera.height is not None:
                    Printer.yellow(
                        f"SLAM: camera size mismatch on load_system_state(): {camera0.width}x{camera0.height} != {self.camera.width}x{self.camera.height}"
                    )
                # update the camera
                self.camera = camera0
                if self.volumetric_integrator is not None:
