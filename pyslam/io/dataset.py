...
class Dataset(object):
    ...
    def getImageColorRight(self, frame_id):
        ...
        try:
            ...
            if self.num_frames is not None and frame_id >= self.num_frames:
                Printer.yellow(
                    f"Dataset end: {self.name}, path: {self.path}, right image, frame id: {frame_id}"
                )
            else:
                Printer.red(
                    f"Cannot open dataset: {self.name}, path: {self.path}, right image, frame id: {frame_id}"
                )
            return np.ascontiguousarray(img) if img is not None else None
...
class FolderDataset(Dataset):
    def __init__(
        ...
        self.listing = glob.glob(
            path + "/" + self.name
        )  # 'name' is used for specifying a glob pattern
        self.listing.sort()
...
class ScannetDataset(Dataset):
    ...
    def __init__(
        ...
        self.ignore_label = 0
        self.label_version = config.dataset_settings[
            "label_version"
        ]  # If nyu40 do nothing, if scannet map them to nyu40
    ...
    def load_scannet_to_nyu40(self, config):
        if "scannetv2_labels_combined_path" in config.dataset_settings:
            ...
            scannet_to_nyu40_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "scannetv2-labels.combined.tsv"
            )
...
class EurocDataset(Dataset):
    def __init__(
        ...
        if name_uc in euroc_timestamps_names:
            ...
            distances = [
                (candidate, levenshtein_distance(name_uc, candidate))
        ...
        if not os.path.exists(euroc_timestamps_file_path):
            Printer.red(
                f"ERROR: [EurocDataset] Cannot find timestamps file: {euroc_timestamps_file_path}!"
            )
            Printer.red(
                f"\t There is a problem in associating the input dataset {name} with the timestamps files in {euroc_timestamps_folder_path}."
            )
            Printer.red(
                f"\t The Euroc sequence name is sopposed to be one of the following: {euroc_timestamps_names}."
            )
